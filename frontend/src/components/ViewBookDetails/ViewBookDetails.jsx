...
const ViewBookDetails = () => {
    ...
    const handleFavourite = async () => {
        const response = await axios.put("https://bs-pro-api.vercel.app/api/v1/add-book-to-favourite",
            {},
            {headers}
        );
    ...
    const handleCart = async () => {
        const response = await axios.put("https://bs-pro-api.vercel.app/api/v1/add-to-cart",
            {},
            {headers}
        );
        alert(response.data.message);
    ...
    const deleteBook = async () => {
        const response = await axios.delete("https://bs-pro-api.vercel.app/api/v1/delete-book",{headers});
        alert(response.data.message);
        navigate("/all-books");
    };
    return (
        <>
            {Data && (
                <div className="px-4 md:px-12 py-8 bg-gradient-to-br from-slate-900 via-gray-600 to-zinc-800 flex flex-col lg:flex-row gap-8">
                    <div className="w-full lg:w-3/6 ">
                        {" "}
                        <div className="flex flex-col lg:flex-row justify-around  bg-gradient-to-tl from-gray-900 via-gray-700 to-gray-500 rounded p-12 ">
